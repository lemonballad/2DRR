...
f=r(:,:);
% p=0.0;SIGMA=[1 p;p 1];
% f=sum(cholcov(SIGMA)*r);
arf=zeros(1,nt);
for ii=0:nt
%     arf=arf+kappa^-2*exp(-((ii*dt)/tau)).*circshift(f,[0,ii]);
    arf=arf+...
        kappa^-2*...
        exp(-kappa^-2*(exp(-(ii*dt)/tau)+(ii*dt)/tau-1)).*...
        circshift(f,[0,ii]);
end
f=(arf);
% p=0.0;SIGMA=[1 p;p 1];
% f=sum(cholcov(SIGMA)*arf);
muf=mean(f);
f=f-muf;
...
% g=cumtrapz(x,cumtrapz(x,xf))/5308/5308;
w_vib=0;
...
% figure;plot(omega/2/pi,real(sig_w/max(real(sig_w))));xlim([-1000 1000]);

% f=fluc*sqrt(2)*exp(-x/tau);f=f-mean(f);
for i1=1:nt
    t1=x/dt-i1;
...
%     ,[fluc fluc],[0 max([max(rsw) max(real(fc))])],'r:','linewidth',2);
% axis square;xlim([0 500]);set(gca,'fontsize',labfont,'linewidth',lwidax);
% xlabel('\omega (cm^{-1})');legend('\sigma','SD',['\Delta = ' num2str(round(fluc)) ' cm^{-1}']);
