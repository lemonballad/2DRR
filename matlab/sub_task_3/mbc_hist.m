...
labfont=16;lwidax=2;
CMass=dlmread('CMass_mbc.txt');omega=dlmread('omega.txt');paramEsts=dlmread('params_mbc.txt');

% Plot Histogram and Distribution for Normal
dist_n=fitdist(CMass,'Normal');
pdf_n=pdf(dist_n,omega);
figure;h1=histogram(CMass,50,'Normalization','pdf','FaceAlpha',1.0,'FaceColor',[1 0 0],'edgecolor',[0 0 0]);
...
figure;h2=histogram(CMass,50,'Normalization','pdf','FaceAlpha',1.0,'FaceColor',[1 0 0],'edgecolor',[0 0 0]);
axis square;xlabel('\omega (cm^{-1})');ylabel('P(\omega)');
title('Probability Distribution <\omega>');set(gca,'fontsize',labfont,'linewidth',lwidax);
