beta=dlmread('C:/Users/Thomas/Desktop/SDs/mbc/mbc_C3.txt',',');
labfont=16;lwidax=2;
[nt,nf]=size(beta);dt=20;nf=2^14; %#ok<ASGLU>
...
fbetay=fftshift(fft(beta.*apod,nf,1),1);fbetay=fbetay/max(max(fbetay));
fbeta=fftshift(fft2(beta.*apod,nf,nf));fbeta=fbeta/max(max(fbeta));

figure;contourf(t(trange),t(trange),beta(trange,trange).*apod(trange,trange),50,'edgecolor','none');
...
load mbc_mbd.mat
y=mbcSS(1:50000);muy=mean(y);y=y-muy;
x=(1:50000)*0.004-0.004;

xc1=xcorr(y,y.^2);xc1=xc1(50000:99999)/muy^3;
xc2=xcorr(y.^2,y);xc2=xc2(50000:99999)/muy^3;
figure;plot(x(1:5:50000),(xc1(1:5:50000)),t,beta(1,:));
figure;plot(x(1:5:50000),(xc2(1:5:50000)),t,beta(:,1));