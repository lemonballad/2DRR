...
path_source='C:\Users\Thomas\Desktop\Gromacs\Output\4-29-16\MBO';
...
L_dihedrals=round(atan2(L_d1,L_d2)*180/pi)+94;
R_dihedrals=round(atan2(R_d1,R_d2)*180/pi)+109;
...
frange=20000:40000;%1:length(freq_PDB);
...
f=fit(time',testcorr','exp2','StartPoint',[0.8 -1/0.07 0.2 -1/12]);%'Lower',[max(testcorr)-0.001 -50],'Upper',[max(testcorr) 1/max(time)]);
...
ffff=apodfun(time,0,0,8,8,false);figure;plot(time,testcorr,time,testcorr.*ffff,time,f1);ylim([-4 5]*10^5);xlim([0 300]);
Nf=length(freq_PDB);
ff=((-1/2:1/Nf:1/2-1/Nf)+1/Nf/2)*2*pi/20/0.00003;
fcorr=real(fftshift(fft(ffff.*testcorr,Nf)));
ff1=real(fftshift(fft(f1,Nf)));
ff2=real(fftshift(fft(f2,Nf)));
f3=real(1./(2.35+1i*ff));
figure;plot(ff,fcorr/max(fcorr),ff,ff1/max(ff1),'Linewidth',3);
legend('filtered data','fit exponentials');xlim([0 100]);xlabel('\omega (cm^-^1)');ylabel('<\omega(0)|\omega(t)>');
end
if false
    file_num=2;
    mbo(file_num).LDA=L_dihedrals';
    mbo(file_num).RDA=R_dihedrals';
    mbo(file_num).freq=freq_PDB;
    mbo(file_num).time=time;
    mbo(file_num).omega=ff;
    mbo(file_num).var=var_freq;
    mbo(file_num).fit=f;
    mbo(file_num).spec_den=fcorr;
    mbo(file_num).corr=testcorr;
    mbo(file_num).apod_param=[0 0 8 8];
    mbo(file_num).frange=frange;
    mbo(file_num).map=freq_map;
end